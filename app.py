...
# ── Camera thread ─────────────────────────────────────────────
def camera_thread():
    global current_frame, current_pred, camera_active, cap, session_words
    ...
    while camera_active:
        ...
        if lm is not None:
            ...
            if len(buf) == 30:
                word, conf = predictor.predict(np.array(buf))
                if conf > 0.70 and word:
                    ...
                    if added:
                        ...
                        # Speak in background (hindi_mode stored in session not accessible in thread,
                        # so we default English; frontend can call /api/tts explicitly)
                        threading.Thread(target=speak, args=(word,), daemon=True).start()
        ...
        # Overlay on frame
        ov = annotated.copy()
        cv2.rectangle(ov,(0,0),(640,56),(6,5,8),-1)
        if current_pred["word"]:
            cv2.putText(ov,
                f"{current_pred['word']}  {current_pred['confidence']}%",
                (18,36), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (240,171,65), 2)
        with camera_lock:
            current_frame = ov
        time.sleep(0.03)
...
def gen_frames():
    while camera_active:
        with camera_lock: frame = current_frame
        if frame is None:
...
@login_req
def dashboard():
    # Build sign dictionary from label map
    try:
...
def stop_camera():
    ...
    return jsonify({"status":"stopped","words":session_words,
                    "sentence":sentence_b.get_sentence(),
                    "duration":duration})
...
@login_req
def nlp_polish():
    """Return a grammatically polished version of the current sentence."""
...
@login_req
def tts_word():
    """Serve TTS audio for a single word as base64 MP3."""
...
@login_req
def tts_sentence():
    """Speak the full current sentence (polished) in English or Hindi."""
    data   = request.get_json()
    lang   = data.get("lang","en")
    words  = sentence_b.get_words()
    ...
    if lang == "hi":
        ...
        path = speak(text, lang="hi")
    else:
        path = speak(text, lang="en")
    if not path or not os.path.exists(path):
        return jsonify({"error":"tts failed"}), 500
    with open(path,"rb") as f:
        audio_b64 = base64.b64encode(f.read()).decode()
